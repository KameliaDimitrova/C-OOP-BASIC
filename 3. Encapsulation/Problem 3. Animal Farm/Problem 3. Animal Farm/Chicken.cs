using System;
using System.Linq;
using System.Xml;

namespace AnimalFarm.Models
{
    public class Chicken
    {
        public const int MinAge = 0;
        public const int MaxAge = 15;

        private string name;
        private int age;

        public Chicken(string name, int age)
        {
            this.Name = name;
            this.Age = age;
        }

        public string Name
        {
            get
            {
                return this.name;
            }

            private set
            {
                this.name = value;
            }
        }

        public int Age
        {
            get
            {
                return this.age;
            }

            private set
            {
              this.age = value;
            }
        }

        private bool IsInputDataValid()
        {
            if (this.Name == null || this.Name == String.Empty || this.Name.All(char.IsWhiteSpace)||this.Age<0||this.Age>15)
                return false;
            return true;
        }
        private double ProductPerDay
        {
            get
            {
                return this.CalculateProductPerDay();
            }
        }

        private double CalculateProductPerDay()
        {
            switch (this.Age)
            {
                case 0:
                case 1:
                case 2:
                case 3:
                    return 1.5;
                case 4:
                case 5:
                case 6:
                case 7:
                    return 2;
                case 8:
                case 9:
                case 10:
                case 11:
                    return 1;
                default:
                    return 0.75;
            }
        }

        public override string ToString()
        {
            if (IsInputDataValid())
                return $"Chicken {Name} (age {Age}) can produce {ProductPerDay} eggs per day.";
            if (this.Name == null || this.Name == String.Empty || this.Name.All(char.IsWhiteSpace))
                return "Name cannot be empty.";
            return "Age should be between 0 and 15.";

        }
    }
    
}
